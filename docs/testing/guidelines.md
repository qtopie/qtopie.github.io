...
## 2. 框架与工具链
- **Test Runner:** 根据 Shell (Bash) / Python 选用标准测试框架（如 Go `testing` + `testify` / Vitest / Jest / PyTest）。
...
## 4. 跑测试的标准命令
- **运行全量单元测试:** 根据 Shell (Bash) / Python 配置对应的单元测试运行指令（例如 `go test -v -race ./...` 或 `npm test`）。
