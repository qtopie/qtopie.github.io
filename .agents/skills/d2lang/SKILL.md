...
- **嵌套容器子节点默认垂直堆叠**：ELK 对嵌套容器默认从上到下堆叠子节点，会形成竖长条、与横向父级不协调。**必须**在容器上使用 `grid-columns: N`（N 为每行子节点数）强制横向排列，使同一逻辑层的节点横向对齐。
- **避免长返回边跨越整图**：反向数据流（如存储层 → 客户端的结果集返回）直接连线会穿过中间层造成交叉；应把回传路由到中途节点（如执行器）再继续向上游，缩短并理顺连线。
